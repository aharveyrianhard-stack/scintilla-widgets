...
test("deck keeps curated navigation, presentation state, and one X pane", () => {
  ...
  assert.match(deck, /const sharedSceneFromDb/);
  assert.match(deck, /active_scene: sharedSceneToDb\(next\.active_scene\)/);
  assert.match(deck, /shared · sign in required/);
  assert.match(deck, /FLEX 3 · shared state unavailable/);
  assert.doesNotMatch(deck.match(/function rememberEditableState[\s\S]*?function rememberPresentationState/)?.[0] || "", /station\.chart\./);
  assert.doesNotMatch(deck, /remember\("station\.scene"/);
  assert.match(deck, /sceneChanged && !options\?\.shared && !namedSceneEditable\(\)/);
  assert.match(deck, /Authorization: "Bearer " \+ SharedStation\.session\.access_token/);
