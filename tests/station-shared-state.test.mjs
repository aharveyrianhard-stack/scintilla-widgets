...
const migration = fs.readFileSync(
  new URL("../supabase/migrations/20260811195733_station_shared_state.sql", import.meta.url),
  "utf8"
);

test("Station shared state separates central choices from local presentation", () => {
  ...
  assert.match(migration, /active_scene text not null default 'live'/i);
  ...
  assert.doesNotMatch(definition, /\b(?:zoom|pan|viewport|presentation)\b/i);
});

test("Station shared state is owner-scoped and never anonymously exposed", () => {
