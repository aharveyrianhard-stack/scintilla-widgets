...
## 2026-08-19T18:38:05Z — the authority sweep: eight equity surfaces, zero unreviewed hosts at runtime
...
## 2026-08-19T18:38:12Z — an axis label is a claim: it must be the price of the line it sits beside
...
## 2026-08-19T18:38:19Z — an empty FAV names itself, and the way out works
...
## 2026-08-19T18:38:29Z — filed defects 1–2 FIXED and browser-verified: cohorts replace the favorites rows
...
## 2026-08-19T18:38:49Z — the pane's direction color is claimed from the day's baseline, or not claimed at all
...
## 2026-08-19T18:38:57Z — the DCF FY baseline rides the history tables; the shortfall stays flagged
...
## 2026-08-19T18:39:04Z — the DCF growth/margin slider seeds ride the FY history; the shortfall stays flagged
...
## 2026-08-19T18:39:16Z — the inventory sweep: runtime coverage completed, and the LINES panel says its library is dead
...
## 2026-08-19T18:39:28Z — the Station WALL refuses the day's direction when the provider states no previous close
...
## 2026-08-19T18:39:34Z — /news and /cohorts main reads: named when never loaded, STALE-stamped when they die mid-life
...
## 2026-08-19T18:39:46Z — /econ, /alerts, /news: dead reads named; empty tables keep their own words
...
## 2026-08-19T18:39:48Z — degraded states, photographed: absence renders as absence
...
## 2026-08-19T18:39:57Z — /pulse: a dead read no longer wears an empty table's words
...
## 2026-08-19T18:39:59Z — the catch-null sweep, browser-verified: failures render as failures
...
## 2026-08-19T18:40:06Z — the realtime lane tells the truth at every stage, on the vendored library
...
## 2026-08-19T18:40:11Z — filed defect 3 FIXED and browser-verified: AI POWER shows its declared rows
...
## 2026-08-19T18:40:12Z — rig smoke + BEFORE state of the filed cohort defects
...
## 2026-08-19T18:40:22Z — the preset scenes paint their declared rows: the first-basket class closed for every scene kind
...
## 2026-08-19T18:40:36Z — a failed main read never claims the owner's absence: /youtube's feed fixed, the axis consumers photographed
...
## 2026-08-19T18:40:50Z — /fundamentals: absent balance fields are flagged by name, not minted into net debt
...
## 2026-08-19T18:40:59Z — the Hub entry: the live path's exact refusal, and the deck booting AT the hub origin
...
## 2026-08-19T18:40:59Z — ticks, repaint and provider-vs-display, browser-verified; CLOSED-label disposition
...
## 2026-08-19T18:41:03Z — an unknown day change gets neither a sign nor a direction colour
...
## 2026-08-19T18:41:08Z — a size is a claim: an unknown SIZE metric is a placeholder, not a measurement
...
## 2026-08-19T18:41:14Z — a position is a claim: a row with no value has not earned a rank
...
## 2026-08-19T18:41:36Z — the watch-later lane tells the truth: unknown is said, lost writes say why
...
## 2026-08-19T18:41:40Z — /youtube: a lost shared write no longer stays painted as saved
...
- **The vendored SDK blocked** (browser-proof/receipts/firebase-sdk-absent-named.png): no app, and the state says which file is missing — "firebase-app compat SDK (vendored same-origin) did not load" — in words a surface can print instead of drawing a confident empty. The config still resolves, so the absence is correctly attributed to the SDK and not to the configuration. Zero page errors: a missing SDK breaks nothing.
- **Only the app half blocked — an upstream load-order hazard, found by running this** (browser-proof/receipts/firebase-analytics-without-app-upstream-throw.png): `firebase-analytics-compat.js` reaches for the app SDK's `INTERNAL` registry at its own top level, so with the app tag failing and the analytics tag succeeding, **the vendored file throws `TypeError: Cannot read properties of undefined (reading 'INTERNAL')` before a line of ours runs**. Nothing in `/_firebase/firebase.js` can prevent it; it is the SDK's own contract. Named here rather than hidden by blocking both tags, because the consequence is operational: **a page must carry the analytics tag only alongside the app tag, never without it.** What is ours still holds — the module names the absence and the page still settles into a coherent, printable state.
- **Auto-start off, one key overridden** (browser-proof/receipts/firebase-app-up-analytics-off.png): exactly one Firebase app exists, `__FIREBASE_CONFIG__ = { projectId: … }` wins for that key alone while the other six keep their values, and analytics never starts — reason: "auto-start disabled by __FIREBASE_ANALYTICS_AUTO__". This is the switch a surface that must not emit uses. Per-key override is this build-stepless tree's stand-in for an env var: `process` does not exist in a browser and nothing here substitutes it at build time, so the guide's bare `process.env.NEXT_PUBLIC_*` reads would have thrown ReferenceError and taken the module with them. The read is guarded, inert here, and starts working by itself if this tree is ever built as `apps/web`.
- **The default path, loaded twice** (browser-proof/receipts/firebase-default-path-settled.png): the app initialises for project `scintilla-834af` on SDK 12.18.0, analytics resolves through the real `isSupported()` to a decided, named outcome, and a **second script tag for the same module adopts the first app rather than throwing `app/duplicate-app`** — `firebase.apps.length` stays 1 and `SC_FIREBASE.app()` is the SDK's own default app. Analytics reaching for its tag host is refused by the rig, which is itself the evidence the lifecycle ran rather than being short-circuited.
