...
Generated against `main` + the visible-truth closure branch. 65 deployed HTML surfaces.
...
`/visuals` and its twelve sub-routes — `/visuals/archive`, `/visuals/bench`, `/visuals/gallery`,
...
`tests/station-route-inventory.test.mjs`, the same way as the surfaces: **183 non-HTML files.**
...
| `/_indicators/station-lens.js` | `/chart` + its shell mirror, by script tag — the Context Lens rule: what the second view shows (the opposite zoom of the same live bars) and where it sits (never over the price line or its newest fifth). Pure functions: no fetch, no DOM, no timer, so a chart that fails to load it simply has no lens |
...
| Documentation (`*.md`) | 10 | the root docs, receipt and handoff, plus the bridge draft's README — served as text |
| Test suites (`*.test.mjs`) | 42 | the tests directory and the bridge draft's — served as text; they are the review evidence |
