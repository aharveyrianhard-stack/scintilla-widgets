...
| Extraction pipeline | 20 | `/_pipeline/` — the isolated YouTube caption→sentiment worker, its offline unit suite and README. Python source served as text and never executed by the deploy, the same as `supabase/`; it reads every credential from the environment, so serving it discloses nothing |
