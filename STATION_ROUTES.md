...
`tests/station-route-inventory.test.mjs`, the same way as the surfaces: **87 non-HTML files.**
...
| Class | Count | What |
| --- | --- | --- |
| Browser proof rig and receipts | 20 | the real-Chromium verification rig, its fixtures, proof scripts, screenshots and receipt log — see `browser-proof/receipts/RECEIPTS.md` |
| Documentation (`*.md`) | 8 | the root docs, receipt and handoff, plus the bridge draft's README — served as text |
| Test suites (`*.test.mjs`) | 27 | the tests directory and the bridge draft's — served as text; they are the review evidence |
