...
`tests/station-route-inventory.test.mjs`, the same way as the surfaces: **201 non-HTML files.**
...
| Class | Count | What |
| --- | --- | --- |
| Browser proof rig and receipts | 93 | the real-Chromium verification rig, its fixtures, proof scripts, screenshots and receipt log — see `browser-proof/receipts/RECEIPTS.md` |
...
| Extraction pipeline | 28 | `/_pipeline/` — the isolated YouTube caption→sentiment worker, its offline unit suite and README. **Probed on the branch preview: `.py` files return 200** with `content-type: application/octet-stream`, `content-disposition: inline`, `access-control-allow-origin: *` and `x-robots-tag: noindex` — served, not executed, same as `supabase/`. It holds no credential (all are read from the environment), but it does disclose production schema and table names, the R2 bucket name and the staging SQL shape — no more than the already-served migrations do, and the same accepted exposure. Excluding it from the deploy is an **OWNER RULING**: a `.vercelignore` would make this walk over-report, since the gate is built on "Vercel serves every file in the repository" |
| Documentation (`*.md`) | 8 | the root docs, receipt and handoff, plus the bridge draft's README — served as text (`/_pipeline/README.md` is counted with its own class above, not here) |
...
| Supabase sources | 10 | edge-function and migration sources — served as text, never executed by the deploy |
