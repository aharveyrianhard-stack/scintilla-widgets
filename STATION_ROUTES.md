...
`/visuals/geiger-live`, `/visuals/geiger-motion`, `/visuals/images`, `/visuals/ladder`,
`/visuals/ladder/mini`, `/visuals/menu`, `/visuals/open`, `/visuals/template`, `/visuals/theme`.
These are the look lab, not Station surfaces; they are inventoried so the count is honest, not
because the Station mounts them.
...
`tests/station-route-inventory.test.mjs`, the same way as the surfaces: **171 non-HTML files.**
...
| Documentation (`*.md`) | 9 | the root docs, receipt and handoff, plus the bridge draft's README — served as text |
| Test suites (`*.test.mjs`) | 32 | the tests directory and the bridge draft's — served as text; they are the review evidence |
